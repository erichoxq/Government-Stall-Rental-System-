...
def Database():
    ...
    # Table for rental records (tenant-stall relationship)
    cursor.execute(
    ...
    # Table for location data (to track tenant presence at stalls)
    cursor.execute(
...
def auditor_dashboard():
    ...
    background_image = Image.open("auditor_bg.png")
...
def auditor_profile():
    ...
    profile_window.geometry("600x600")  # Set the size of the new window
    ...
    if username:
        ...
        if auditor_info:
            ...
            profile_image_label = Label(profile_frame, bg="#C7D3FF")
            profile_image_label.pack(pady=(0, 10), anchor='center')
            ...
            # Create a frame for auditor information
            info_frame = Frame(profile_frame, bg="#C7D3FF")
            info_frame.pack(pady=10)

            # Display auditor information with labels and values side by side
            Label(info_frame, text="Username: ", bg="#C7D3FF", font=("Arial", 12, 'bold')).grid(row=0, column=0,
                                                                                                sticky='w')
            Label(info_frame, text=username, bg="#C7D3FF", fg="blue", font=("Arial", 12)).grid(row=0, column=1,
                                                                                               sticky='w')

            Label(info_frame, text="Full Name: ", bg="#C7D3FF", font=("Arial", 12, 'bold')).grid(row=1, column=0,
                                                                                                 sticky='w')
            Label(info_frame, text=fullname, bg="#C7D3FF", fg="blue", font=("Arial", 12)).grid(row=1, column=1,
                                                                                               sticky='w')

            Label(info_frame, text="IC: ", bg="#C7D3FF", font=("Arial", 12, 'bold')).grid(row=2, column=0, sticky='w')
            Label(info_frame, text=ic, bg="#C7D3FF", fg="blue", font=("Arial", 12)).grid(row=2, column=1, sticky='w')

            Label(info_frame, text="Date of Birth: ", bg="#C7D3FF", font=("Arial", 12, 'bold')).grid(row=3, column=0,
                                                                                                     sticky='w')
            Label(info_frame, text=str(date_of_birth), bg="#C7D3FF", fg="blue", font=("Arial", 12)).grid(row=3,
                                                                                                         column=1,
                                                                                                         sticky='w')

            Label(info_frame, text="Email: ", bg="#C7D3FF", font=("Arial", 12, 'bold')).grid(row=4, column=0,
                                                                                             sticky='w')
            Label(info_frame, text=email_address, bg="#C7D3FF", fg="blue", font=("Arial", 12)).grid(row=4, column=1,
                                                                                                    sticky='w')

            Label(info_frame, text="Phone Number: ", bg="#C7D3FF", font=("Arial", 12, 'bold')).grid(row=5, column=0,
                                                                                                    sticky='w')
            Label(info_frame, text=phone_number, bg="#C7D3FF", fg="blue", font=("Arial", 12)).grid(row=5, column=1,
                                                                                                   sticky='w')

            # Center the information frame
            info_frame.pack(pady=10)
    ...
    def admin_profile():
        ...
        if username:
            ...
            if admin_info:
                ...
                # Display admin information with labels and values side by side
                Label(info_frame, text="Username: ", bg="#C7D3FF", font=("Arial", 12, 'bold')).grid(row=0, column=0,
                                                                                                    sticky='w')
                Label(info_frame, text=username, bg="#C7D3FF", fg="blue", font=("Arial", 12)).grid(row=0, column=1,
                                                                                                   sticky='w')
                ...
                Label(info_frame, text="IC: ", bg="#C7D3FF", font=("Arial", 12, 'bold')).grid(row=2, column=0,
                                                                                              sticky='w')
                Label(info_frame, text=ic, bg="#C7D3FF", fg="blue", font=("Arial", 12)).grid(row=2, column=1,
                                                                                             sticky='w')

                Label(info_frame, text="Date of Birth: ", bg="#C7D3FF", font=("Arial", 12, 'bold')).grid(row=3,
                                                                                                         column=0,
                                                                                                         sticky='w')
                Label(info_frame, text=str(date_of_birth), bg="#C7D3FF", fg="blue", font=("Arial", 12)).grid(row=3,
                                                                                                             column=1,
...
def update_evidence_status(evidence_id, status, distance_meters=10):
    ...
    if assignment:
        ...
        # Step 2: Get the attendance_records_id and tenant_id from auditor_assignments
        ...
        if assignment_details:
            ...
            # Step 3: Get the stall_id based on tenant_id from the rental table
            ...
            if rental_record:
                ...
                if stall_location:
                    ...
                    # Step 5: Calculate new latitude and longitude based on the distance
                    ...
                    # Step 6: Update attendance_records table with the new values
                    attendance_status = "Correct Location" if status == "approve" else "Wrong Location"
                    cursor.execute("""
                        UPDATE attendance_records 
                        SET latitude = ?, longitude = ?, attendance_status = ? 
                        WHERE attendance_records_id = ?
                    """, (new_latitude, new_longitude, attendance_status, attendance_records_id))
                    conn.commit()

    # Step 7: Update the evidence status
    cursor.execute("UPDATE evidence SET status = ? WHERE evidence_id = ?", (status, evidence_id))
    conn.commit()

    # Step 8: Fetch attendance_time for the notification and email, if attendance_records_id is defined
...
def generate_report_action(report_type, selected_year):
    ...
    try:
        if report_type == "Monthly Payment Report":
            ...
            current_date = datetime.date.today()
            current_year_month = current_date.strftime("%B %Y")  # Format month as full name, e.g., "November 2024"

            # Query unpaid tenants up to the current month and year
            cursor.execute("""
                SELECT t.fullname, p.tenant_id, p.paid_periods
                FROM payment p
                JOIN tenant t ON p.tenant_id = t.tenant_id  -- Join with tenant table to get the full name
                WHERE p.payment_status = 'unpaid'
            """)
            data = cursor.fetchall()

            # Add title at the top of the report
            report_lines = [f"Unpaid Tenants Report up to {current_year_month}"]
            ...
            for row in data:
                ...
                if period_start_date.strftime("%m-%Y") <= current_date.strftime("%m-%Y"):
                    report_lines.append(
...
def index():
    ...
    return render_template('test2.html')  # This will serve the map
...
def tenant_dashboard():
    ...
    def stall():
        global attendance_button, stall_frame  # Declare attendance_button as global to modify its state in take_attendance
        username = logged_in_user.get('username')
        if username is None:
        ...
        if rental_info:
            ...
            details_frame = Frame(stall_frame, bg="#C7D3FF")
            details_frame.pack(pady=10, anchor='center')

            details = [
                ("Stall ID:", stall_id),
                ("Location:", location),
                ("Size:", size),
                ("Rent Price per Month:", f"RM{rent_price_per_month:.2f}"),
                ("Rental Period:", f"{start_date} to {end_date if end_date else 'Ongoing'}")
            ]

            for label_text, value in details:
                row_frame = Frame(details_frame, bg="#C7D3FF")
                row_frame.pack(pady=5, anchor='center')
                Label(row_frame, text=label_text, font=('Arial', 12, 'bold'), bg="#C7D3FF", fg="blue").pack(side='left',
                                                                                                            padx=5)
                Label(row_frame, text=value, font=('Arial', 12, 'bold'), bg="#C7D3FF", fg="black").pack(side='left',
                                                                                                        padx=5)

            # Create the attendance button
            attendance_button = ctk.CTkButton(stall_frame, text="Take Attendance", command=take_attendance,
            ...
            attendance_history_button = Label(
                stall_frame, text="View Attendance History", bg="#C7D3FF", fg="darkblue", font=("Arial", 10)
            )
            ...
            if attendance_taken:
                ...
                message_label = Label(stall_frame, text=message, font=('Arial', 12, 'italic'), bg="#C7D3FF", fg="red")
            ...
            def twinkle():
                ...
                next_color = "#C7D3FF" if current_color == "#FF0000" else "#FF0000"
    ...
    def apply_stall():
        ...
        def display_stalls(stalls):
            ...
            for stall in stalls:
                ...
                ctk.CTkLabel(details_frame, text=f"Stall ID: {stall_id}", font=ctk.CTkFont(size=12)).pack(anchor='w')
                ctk.CTkLabel(details_frame, text=f"Price: RM{rent_price:.2f}/month", font=ctk.CTkFont(size=12)).pack(
                    anchor='w')
                ctk.CTkLabel(details_frame, text=f"Size: {size}", font=ctk.CTkFont(size=12)).pack(anchor='w')
    ...
    def feedback():
        ...
        form_frame = Frame(fd_center_frame, bg="#C7D3FF")
        form_frame.pack(side='left', padx=10, fill='y')

        Label(form_frame, text="Feedback:", font=('Algerian', 16, 'bold'), bg="#C7D3FF").pack(pady=10)
        ...
        history_frame = Frame(fd_center_frame, bg="#C7D3FF")
        history_frame.pack(side='left', padx=10, fill='y')

        Label(history_frame, text="Feedback History:", font=('Algerian', 16, 'bold'), bg="#C7D3FF").pack(pady=10)

        # Create a frame for filters at the bottom
        filter_frame = Frame(history_frame, bg="#C7D3FF")
        ...
        # Date selection
        Label(filter_frame, text="Select Date:", bg="#C7D3FF").pack(side='left', padx=5)
        ...
        text_frame = Frame(history_frame, bg="#C7D3FF")
    ...
    def profiles():
        ...
        if username:
            ...
            if tenant_info:
                ...
                # Display the face ID image at the top
                face_image_label = Label(profiles_frame, bg="#C7D3FF")
                face_image_label.pack(pady=(0, 10), anchor='center')
                ...
                info_frame = Frame(profiles_frame, bg="#C7D3FF")
                info_frame.pack(pady=10)

                # Display tenant information with labels and values side by side
                Label(info_frame, text="Username: ", bg="#C7D3FF", font=("Arial", 12, 'bold')).grid(row=0, column=0,
                                                                                                    sticky='w')
                Label(info_frame, text=username, bg="#C7D3FF", fg="blue", font=("Arial", 12)).grid(row=0, column=1,
                                                                                                   sticky='w')
...
def auditor_account(ic_number=None):
    ...
    username_entry.insert(0, "Enter auditor username")
    username_entry.config(fg='grey')
    username_entry.bind("<FocusIn>", lambda event: on_entry_click(username_entry, "Enter auditor username"))
    username_entry.bind("<FocusOut>", lambda event: on_focus_out(username_entry, "Enter auditor username"))
    ...
    password_entry.insert(0, "Enter auditor password")
    password_entry.config(fg='grey')
    password_entry.bind("<FocusIn>", lambda event: password_on_focus_in(password_entry, "Enter auditor password"))
    password_entry.bind("<FocusOut>", lambda event: password_on_focus_out(password_entry, "Enter auditor password"))
    ...
    confirm_password_entry = Entry(AuditorFrame, font=('times new roman', 15), width=20, highlightbackground="black",
                                   highlightcolor="black", highlightthickness=1)
    confirm_password_entry.grid(row=4, column=1, padx=10)
    confirm_password_entry.insert(0, "Confirm auditor password")
    confirm_password_entry.config(fg='grey')
    confirm_password_entry.bind("<FocusIn>",
                                lambda event: password_on_focus_in(confirm_password_entry, "Confirm auditor password"))
    confirm_password_entry.bind("<FocusOut>",
                                lambda event: password_on_focus_out(confirm_password_entry, "Confirm auditor password"))
    ...
    fullname_entry.insert(0, "Enter auditor full name")
    fullname_entry.config(fg='grey')
    fullname_entry.bind("<FocusIn>", lambda event: on_entry_click(fullname_entry, "Enter auditor full name"))
    fullname_entry.bind("<FocusOut>", lambda event: on_focus_out(fullname_entry, "Enter auditor full name"))
    ...
    email_entry.insert(0, "Enter auditor email address")
    email_entry.config(fg='grey')
    email_entry.bind("<FocusIn>", lambda event: on_entry_click(email_entry, "Enter auditor email address"))
    email_entry.bind("<FocusOut>", lambda event: on_focus_out(email_entry, "Enter auditor email address"))
    ...
    phone_entry.insert(0, "Enter auditor phone number")
    phone_entry.config(fg='grey')
    phone_entry.bind("<FocusIn>", lambda event: on_entry_click(phone_entry, "Enter auditor phone number"))
    phone_entry.bind("<FocusOut>", lambda event: on_focus_out(phone_entry, "Enter auditor phone number"))
...
def register_auditor():
    ...
    try:
        placeholders = {
            "USERNAME_REGISTER": "Enter auditor username",
            "PASSWORD_REGISTER": "Enter auditor password",
            "FULLNAME": "Enter auditor full name",
            "IC": "Enter auditor IC",
            "DATE_OF_BIRTH": "Enter day-month-year",
            "EMAIL_ADDRESS": "Enter auditor email address",
            "PHONE_NUMBER": "Enter auditor phone number"
        }
